...
<html>
    <head>
        ...
        <link rel="stylesheet" href="thongke1.css?v= <?php echo time(); ?>">
    </head>
    <body>
        <div class="container">
            <header class="row">
            ...
            <div class="row">
                ...
                <?php
                    ...
                    $sql ="SELECT (SUM(TIENDV) + (SELECT SUM(t1.SOLUONG * t2.GIAPK)  FROM tbdonhang_detail t1 INNER JOIN tbphukien t2 ON t1.MAPK = t2.MAPK WHERE t1.NGAY = CURRENT_DATE)) as tongtien FROM tbdonhang WHERE tbdonhang.NGAY = CURRENT_DATE and tbdonhang.TRANGTHAI = 1";
                    $result = $conn->query($sql);
                    if($result->num_rows > 0){
                        while($row = $result->fetch_assoc()){
                    ?>
                            <h1>Tổng tiền trong ngày:<?php echo $row["tongtien"] ?></h1> 
                    <?php 
                        }
                    }
                    $conn->close();
